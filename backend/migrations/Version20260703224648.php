...
final class Version20260703224648 extends AbstractMigration
{
    ...
    {
        $this->addSql('CREATE TABLE refresh_token (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, token_hash VARCHAR(64) NOT NULL, expires_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, revoked_at TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT NULL, created_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, replaced_by_id INTEGER DEFAULT NULL, user_id INTEGER NOT NULL, CONSTRAINT FK_C74F21959AC69B54 FOREIGN KEY (replaced_by_id) REFERENCES refresh_token (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_C74F2195A76ED395 FOREIGN KEY (user_id) REFERENCES "user" (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE)');
